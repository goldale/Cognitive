...
dump(CAN/'terminology.yaml', {'kind': 'T_Collection', 'schema_version': '0.3.16', 'terms': terms})
...
changes=['# CHANGES — cognitive 0.3.16','', 'This incremental release introduces a complete architectural refactoring around Semantic Feedback Learning.','', '## Architectural changes','']
...
pp=ROOT/'pyproject.toml'; txt=pp.read_text(); txt=txt.replace('version = "0.3.14"','version = "0.3.16"'); pp.write_text(txt)
mk=ROOT/'Makefile'; txt=mk.read_text().replace('cognitive-systems-lab-0.3.14.tar.gz','cognitive-systems-lab-0.3.16.tar.gz'); mk.write_text(txt)
br=ROOT/'scripts'/'build_release.sh'; txt=br.read_text().replace('cognitive-systems-lab-0.3.14.tar.gz','cognitive-systems-lab-0.3.16.tar.gz'); br.write_text(txt)
manifest=yaml.safe_load((STATE/'manifest.yaml').read_text()); manifest['version']='0.3.16-stage2'; dump(STATE/'manifest.yaml',manifest)
