...
idx=(ROOT/'docs/chapter26/index.html').read_text(errors='replace') if (ROOT/'docs/chapter26/index.html').exists() else ''
...
result={'version':'0.3.19-stage4','summary':{'total':len(checks),'pass':len(checks)-len(failed),'fail':len(failed),'status':'PASS' if not failed else 'FAIL'},'checks':checks}
