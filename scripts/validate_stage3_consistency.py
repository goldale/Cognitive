...
# A-Z coverage
idx=(DOCS/'chapter26/index.html').read_text(errors='replace')
...
out={'version':'0.3.19-stage3','summary':{'total':len(checks),'pass':len(checks)-len(failed),'fail':len(failed),'status':'PASS' if not failed else 'FAIL'},'checks':checks}
