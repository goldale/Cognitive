...
check('manifest release',m.get('release')=='0.3.34',str(m.get('release')))
check('manifest version',m.get('version')=='0.3.34',str(m.get('version')))
check('release notes target',(ROOT/m.get('release_notes','')).is_file(),m.get('release_notes',''))
check('pyproject version','version = "0.3.34"' in (ROOT/'pyproject.toml').read_text())
for p in ['README.md','CHANGES.md','ARCHITECTURE_FREEZE.md','RELEASE_MANIFEST.json','RELEASE_NOTES_0.3.34.md','state','docs','src','tests','scripts']:
...
check('build release version','0.3.34' in b)
# canonical docs
...
out={'release':'cognitive-0.3.34','stage':7,'status':'PASS' if not failed else 'FAIL','checks':checks,'summary':{'pass':len(checks)-len(failed),'fail':len(failed)}}
