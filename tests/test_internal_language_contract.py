...
def test_shared_language_communication_contract():
    assert "same internal language" in texts("04_03.yaml")
    assert "never enter LTM1 or LTM2" in texts("04_03.yaml")
    assert "reconstruction of a compatible cognitive state" in texts("04_03.yaml")
    assert "D-Context defines the serialization subset" in texts("04_04.yaml")
    assert "first exchange" in texts("04_05.yaml")


def test_time_critical_and_chess_examples():
    assert "carrier of the cognitive system may cease to exist" in texts("07_05.yaml")
    assert "chess move" in texts("09_01.yaml")


def test_memory_architecture_shared_language_cycle():
    data = load("10_01.yaml")
    ...
    assert "same Transformer-derived internal language" in texts("10_01.yaml")
